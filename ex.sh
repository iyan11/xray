...
generate_link() {
    ...
    then
        ...
        link="vless://${id}@${address}:${port}?fragment=&security=reality&encryption=none&pbk=${public_key}&fp=chrome&type=tcp&flow=xtls-rprx-vision-udp443&sni=${server_name}&sid=${short_id}#xray-ivan"
    else # grpc config
        id=$(strip_quotes $(jq ".outbounds[0].settings.vnext[0].users[0].id" $1))
        address=$(strip_quotes $(jq ".outbounds[0].settings.vnext[0].address" $1))
        service_name=$(strip_quotes $(jq ".outbounds[0].streamSettings.grpcSettings.serviceName" $1))
        link="vless://${id}@${address}:443?security=tls&encryption=none&fp=chrome&type=grpc&serviceName=${service_name}#xray-ivan-CDN"
    fi
    echo -e "$link"
}
